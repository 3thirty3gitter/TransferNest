...
export async function multiAgentRespond(opts: { userInput: string; sessionId?: string; maxRetries?: number }) {
  const { userInput, sessionId = 'default', maxRetries = 2 } = opts;

  const memorySnippets = await retrieveTopLessons(userInput, 5);

  const systemPreamble = [
    `You are a multi-agent software dev team (Architect, Backend, Frontend, Data/Sec, QA, Ops).`,
    `First, you MUST review the operational rules below and adhere to ops/RESPONSE_SCHEMA.json.`,
  ...
  const prompt = [
    ...
    rulesBlock,
    '\n\n# Task\n' + userInput,
    '\n\n# Output Contract\nReturn a single JSON object that conforms to ops/RESPONSE_SCHEMA.json. Include a rule_checklist with PASS/FAIL for mandatory rules. If any FAIL → status="STOP" and explain.'
  ].join('\n');
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    ...
    const text = gen.response.candidates?.[0]?.content?.parts?.map((p: any) => p.text).join('') || '';
    const json = sniffJson(text);

    if (!json) {
      lastError = new Error('Model did not return valid JSON');
    } else if (!validate(json)) {
      lastError = new Error('Schema validation failed: ' + ajv.errorsText(validate.errors));
    } else {
      try {
        const writes = json?.memory?.write as { lesson: string; tags: string[]; importance: 'low'|'medium'|'high' }[] | undefined;
    ...
    const repairMsg = [
      ...
      'Repair by returning ONLY a JSON object that validates against ops/RESPONSE_SCHEMA.json. Do not include markdown or commentary.'
    ].join('\n');
    ...
    const repairText = genRepair.response.candidates?.[0]?.content?.parts?.map((p: any) => p.text).join('') || '';
    const repairJson = sniffJson(repairText);
    if (repairJson && validate(repairJson)) {
      try {
        const writes = repairJson?.memory?.write as { lesson: string; tags: string[]; importance: 'low'|'medium'|'high' }[] | undefined;
        if (writes?.length) await Promise.all(writes.map(w => saveLesson(w)));
      } catch {}
      return repairJson;
    }
...
export async function retrieveTopLessons(query: string, k = 5): Promise<Array<{ id: string; lesson: string }>> {
  const snap = await db.collection('ai_lessons').orderBy('createdAt', 'desc').limit(100).get();
...
export async function saveLesson(w: { lesson: string; tags: string[]; importance: 'low'|'medium'|'high' }) {
  await db.collection('ai_lessons').add({ ...w, createdAt: admin.firestore.FieldValue.serverTimestamp() });
}
...
function sniffJson(text: string): any | null {
